...
```{r}
# Perform an inner join between `flights` and `weather` datasets.
...
## Merges
In this section, we will merge the `flights` dataset with the `weather` dataset from the `nycflights13` package to analyze how weather conditions might have affected the flights.
...
#### Inner Merge
In an inner merge, only the rows with matching keys in both data frames are returned. Rows with non-matching keys are excluded from the result. It's useful when you want to merge datasets based on common key columns, and you are only interested in rows with matching keys in both datasets.

```{python}
# Perform an inner merge between `flights` and `weather` datasets.
flights_weather_inner_merged = pd.merge(flights, weather, how = "inner", on = ["year", "month", "day", "hour", "origin"])
```

#### Left Merge
A left merge returns all rows from the left dataset and the matched rows from the right dataset. If there is no match found in the right dataset, then the result will contain `NA`. Use a left merge when you want to retain all records from the "left" dataset, and add matching records from the "right" dataset where available.
```{python}
# Perform a left merge between `flights` and `weather` datasets.
flights_weather_left_merged = pd.merge(flights, weather, how = "left", on = ["year", "month", "day", "hour", "origin"])
```

#### Right Merge
In a right merge, all rows from the right dataset and the matched rows from the left dataset are returned. If there is no match found in the left dataset, then the result will contain `NA`. It is the opposite of a left merge and is used when you want to retain all records from the "right" dataset.
```{python}
# Perform a right merge between `flights` and `weather` datasets.
flights_weather_right_merged = pd.merge(flights, weather, how = "right", on = ["year", "month", "day", "hour", "origin"])
```

#### Full Merge
A full merge returns all rows when there is a match in either the left or right dataset. If there is no match found in either dataset, then the result will contain `NA`. It is useful when you want to retain all records from both datasets.
```{python}
# Perform a full merge between `flights` and `weather` datasets.
...
#### Using Merges to Analyze the Data
```{python}
# Use the inner merged dataset to calculate the average departure delay for flights with precipitation greater than 0.5.
...
#### Anti Merge
An anti merge returns rows from the left dataset where there are no matching keys in the right dataset. It's useful for identifying records in one dataset that do not have a counterpart in another dataset.
```{python}
# The indicator argument shows which dataset the row came from and is needed for anti-merge.
