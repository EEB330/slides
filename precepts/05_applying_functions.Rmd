...
Let's re-create the original plant heights dataset in Python. We will use the `pandas` library to create a DataFrame with the same structure as the R dataset.
```{python}
import pandas as pd
```
...
Instead of using `apply`, we will use the `axis` parameter to define the direction of the operation. Here, we will calculate the mean height of each plant across the three time points.
```{python}
# Calculate the mean height for each row in the data frame
row_means = plant_data.iloc[:, 1:].mean(axis = 1)
...
added_constant = map(1:10, add_nums, 5)
...
- **Expected Output:** Two data frames, one with the mean values and the other with the variance values for the specified columns (1 row and 3 columns, each).
...
### Exercise 2 -- Explorting 3D Iris Data *in R*
...
- **Task 1:** Calculate the average sepal length, sepal width, and petal length across individuals for each species in the `iris` dataset.
...
- **Expected Output:** A `data.frame` with the variance in sepal area for each species (1 row and 3 columns)
